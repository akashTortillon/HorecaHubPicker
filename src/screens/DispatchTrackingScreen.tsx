...
const DispatchTrackingScreen = () => {
  ...
  const fetchDispatches = useCallback(
    async (pageNum: number, append = false) => {
      // Guard: Don't call if already loading or if we've reached the end (unless it's a refresh)
      ...
      try {
        ...
        const mapped: DispatchItem[] = items.map((item: any) => ({
          ...
          driver_name: item.delivery_agent_name,
        ...
        // Only update current page state on SUCCESS
        setPage(pageNum);
        // If we got fewer items than the page size, there are no more pages
        setHasMore(mapped.length === pageSize);
      } catch (error) {
        console.error('Failed to fetch dispatches:', error);
        // Stop further auto-pagination on error to prevent infinite loops
        setHasMore(false);
      } finally {
        setLoading(false);
  ...
  const onRefresh = () => {
    setRefreshing(true);
    setHasMore(true);
    fetchDispatches(1, false);
  };

  const loadMore = () => {
    // Only trigger next page if not currently doing any loading and more exists
    if (!loading && !loadingMore && hasMore) {
      fetchDispatches(page + 1, true);
    }
  };
    ...
    <SafeAreaView style={styles.container}>
      ...
      {loading && page === 1 ? (
        ...
        <FlatList
          ...
          onEndReachedThreshold={0.1} // Reduced threshold to prevent aggressive calls
          refreshControl={
          ...
          ListEmptyComponent={
            !loading ? <Text style={styles.emptyText}>No dispatches found</Text> : null
          }
        />
      )}
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  orderId: {fontSize: 14, color: '#64748B', fontWeight: '700', marginTop: 2},
  statusTag: {
  ...
  statusTagText: {color: '#FFFFFF', fontSize: 11, fontWeight: '800'},
  detailsBox: {backgroundColor: '#F8FAFC', borderRadius: 16, padding: 16, gap: 12},
  detailRow: {flexDirection: 'row', alignItems: 'center', gap: 12},
  detailText: {fontSize: 15, fontWeight: '700', color: '#1E293B', flex: 1},
